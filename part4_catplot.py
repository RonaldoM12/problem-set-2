...
def catplot(df):
    ...
    g.set_axis_labels("Charge type", "Predicted Probability of Felony Rearrest")
    plt.title('Predicted felony rearrest by charge type')
    plt.savefig('./data/part4_plots/felony_rearrest_by_charge.png')
    plt.close()
...
def nonfelony_arrest_catplot(df):
    sns.set(style= "whitegrid")
    g = sns.catplot(
        data= df,
        x = "charge_degree",
    ...
    g.set_axis_labels("Charge type", "Predicted Probability of Felony Rearrest")
    plt.title('Predicted Non-Felony rearrest by Charge Type')
    plt.savefig('./data/part4_plots/nonfelony_rearrest_by_charge.png')
    plt.close()
...
def repeated_catplot(df):
    ...
    g.set_axis_labels("Charge type", "Predicted Probability of Felony Rearrest")
    plt.title('Predicted Felony rearrest by Charge Type and actual outcome')
    plt.savefig('./data/part4_plots/felony_rearrest_by_charge_and_outcome.png')
    plt.close()
