...
def extract_transform():
    ...
    felony_charge = arrest_events.groupby('arrest_id').apply(
        lambda group: pd.Series({'has_felony_charge': (group['charge_degree'] == 'F').any()})
    ).reset_index

    pre_universe_felony = pred_universe.merge(felony_charge, on= 'arrest_id', how='left')
    pre_universe_felony['has_felony_charge'].fillna(False, inplace=True)
    
    return pred_universe, arrest_events, charge_counts, charge_counts_by_offense, felony_charge, pre_universe_felony
